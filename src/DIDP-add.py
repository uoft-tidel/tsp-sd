...
def main (fpath, opt, export_results = False):
    ...
    del_arc = [[set([0]) for j in range(n)] for i in range(n)]
    ...
    for j in range(1, n):
        visit = dp.Transition(
            ...
            #add: len(d[location,j].difference(unvisited)) > 0
            preconditions=[unvisited.contains(j), d[location,j].difference(unvisited.add(0)).len() > 0], #, first != 0],
            effects=[
