...
if __name__ == "__main__":
    ...
    for instance in [f for f in os.listdir(instance_folder) if batch in f]:
        ...
        csv_remain.append([ntpath.basename(fname), n-1, len(never_deleted_edges), len(never_deleted_set)])
...
with open('removed_options.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerows(csv_remain)